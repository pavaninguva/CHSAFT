...
class InitialConditions(UserExpression):
    ...
    def eval(self, values, x):
        ...
        values[0] = ln((A_RAW) + 2.0 * (NOISE_MAGNITUDE) * (0.5 - random.random()))
        values[1] = 0.0
...
if GIBBS == "FH":
    ...
    g = (x_a * exp(x_a) / N_A ) + ((1.0-exp(x_a)) * ln(1.0 - exp(x_a)) / N_B) + exp(x_a) * (1.0 - exp(x_a))*chi_AB
...
L_1 = (
    x_a * h_1 * dx
    - x_a0 * h_1 * dx
    + dt * exp(x_a) * (1.0 - exp(x_a)) * dot(grad(mu_AB_mid), grad(h_1/exp(x_a))) * dx
)

L_12 = (
    mu_AB * j_1 * dx
    - dgdx_a * (1/exp(x_a)) * j_1 * dx
    - kappa * dot(grad(exp(x_a)), grad(j_1)) * dx
)
