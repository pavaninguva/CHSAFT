...
class LLESolvers(object):
 ...
 def LLE(self,Temp=[298],Pre=[1e5]):
     if self.Solver="vonSolms":
         r = vonSolmsLLE(Method,Species,Length):
         x = r.LLE(Temp,Pre)
     else
         r = GibbsTP(Method,Species,Length):
         x = r.LLE(Temp,Pre)
     return x

class vonSolmsLLE(object):
 def __init(self,Method,Species,Length):
     self.Method = Method
     self.Species = Species
     self.Length = Length
 ...
 def LLE(self,Temp=[298],Pre=[1e5]):
     ...
     x1 = [xSpn1.x[0],1.]
     x2 = [xSpn2.x[0],1.]
     while (abs(x2[0]-x2[1])>1e-6 and abs(x1[0]-x1[1])>1e-6) or i<100:
         if (i % 2)==0:
             x2[0]=x2[1]
